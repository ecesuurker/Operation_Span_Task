...
class Operation_Span_Task():
    ...
    def __init__(self, ParticipantNumber, experimentWindow): #We 
        self.pNum = ParticipantNumber #initialize the variables that we will use throughout
        self.expWind = experimentWindow 
        ...
        field = ["ID", "trial_name", "block", "stimuli", "answers"]
    ...
    def Instructions(self, fileName):
        ...
        sent.draw()
        self.expWind.flip()
        event.waitKeys(keyList=["enter"]) #The instructions appear on the screen until the
    ...
    def NonLinguistic_OST(self, stimuliFile, directory):
        """
    ...
    def Linguistic_OST(self, stimuliFile, trialType, p=0):
        ...
        for block in self.blocknumbers:
            ...
            for i in range(block):
                ...
                self.expWind.flip()
                self.isi.complete()
                w = visual.TextStim(win = self.expWind, color=txtColor, text=words[p], 
                               pos=(0, 0))
                self.isi.start(1)
                w.draw()
                self.expWind.flip()
    ...
    def Run(self):
        ...
        direc = "C:\Users\ece-s\OneDrive\Masaüstü\Senior Project\Operation Span Task\N-OST_Images"
        mathNum = self.NonLinguistic_OST("N-OST_Stimuli.txt", direc)
        L-OST_T = ["TurkishDaily", "EnglishDaily", "TurkishAcademic", "EnglishAcademic"]
        random.shuffle(L-OST_T)
        for trial in L-OST_T:
            s = trial + ".txt"
